...
//                    4=ColorSegmentation
//                    5=LightSegmentation  
  int segmentationMode = 0;
  
  SIPLib(PApplet parent, int w, int h, int SegmentationMode)
  {
    println("SIPLib Sistemi Interattivi Processing Library " + version + " (" + w + "x" + h + "), 2020 by Rudy Melli http://www.vision-e.it/si");
  ...
  void analyze(PImage img_input, int threshold, int areamin, color colTrack, int motion_threshold, int motion_areamin)
  {
    PImage img = img_input.copy();
    if(img.width != this.width || img.height != this.height)
      img.resize(this.width, this.height);

    if(segmentationMode == 0)
      shadowSegmentation(img, threshold, areamin);
    else if(segmentationMode == 1)
      singleDifference(img, threshold, areamin);
    else if(segmentationMode == 2)
      backgroundSuppression(img, threshold, areamin, true, motion_threshold, motion_areamin);
    else if(segmentationMode == 3)
      backgroundSuppression(img, threshold, areamin, false, motion_threshold, motion_areamin);
    else if(segmentationMode == 4)
      colorSegmentationPixel(img, colTrack, threshold, areamin);
    else if(segmentationMode == 5)
      lightSegmentation(img, threshold, areamin);
  }
  void analyze(PImage img_input, int threshold, int areamin)
  {
    analyze(img_input, threshold, areamin, 0, 40, 50);
  }
  ...
  PVector GetFootsPoint(Rectangle rc)
  {
    return new PVector(rc.x + rc.width/2, rc.y + rc. height);
  }
  ...
  float analize(int thAreaMin, boolean bStoreData)
  {
    ...
    float TX = 0, TY = 0;
    int nCog = 0;
    float xl = opencv.width; 
    float xr = 0; 
    float yt = opencv.height; 
    float yb = 0; 
    for (int i=0; i<contours.size(); i++)
    {
      ...
      if(a >  thAreaMin)
      {
        ...
        ++nCog;
        if(r.x < xl) xl = r.x;
        if(r.x + r.width > xr) xr = r.x + r.width;
        if(r.y < yt) yt = r.y;
        if(r.y + r.height > yb) yb = r.y + r.height;
      }
    }
    ...
    if(bStoreData)
    {
      if(areatot >  thAreaMin)
      {
        ...
        boundingbox = new Rectangle((int)xl, (int)yt, (int)(xr - xl), (int)(yb - yt));
        foots = new PVector((xr + xl) / 2.0, yb);
      }
