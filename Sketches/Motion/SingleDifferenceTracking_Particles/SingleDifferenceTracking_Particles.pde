...
void draw() {
  ...
  TrackX = (int)silib.cog.x * 2;
  TrackY = (int)silib.cog.y * 2;
  if(silib.cogs.size() > 0)
    image(silib.imgSegment, 320, 0);
  ...
  println("area=" + silib.area + " motion=" + silib.area_motion + "  x=" + TrackX + " y =" + TrackY);
...
void captureEvent(Capture c) {
  c.read();
  //silib.singleDifference(c, 40, 50);
  silib.backgroundSuppression(c, 40, 50, true, 40, 50);
}
