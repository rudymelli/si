...
void draw() {
  ...
  fill(0, 255, 0);
  textSize(20);
  text("mm:" + rhx + ";" + rhy + "  pixels:" + rhx_px + ";" + rhy_px, 20, 60);
}
...
void sendData() 
{
  ...
  OscMessage myMessage6 = new OscMessage("/head_trackjointpos");
  ...
  myMessage6.add(1); // Coordinate relative al torso
...
void oscEvent(OscMessage theOscMessage) 
{
  ...
  if (v.equals("/head_pos_body")) 
  {
    hx_b = theOscMessage.get(0).floatValue();  
    hy_b = theOscMessage.get(1).floatValue();
    hz_b = theOscMessage.get(2).floatValue();
  }
